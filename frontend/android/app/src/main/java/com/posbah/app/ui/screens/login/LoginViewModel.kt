...
class LoginViewModel @Inject constructor(
    ...
    fun updatePin(p: String) = _uiState.update { it.copy(pin = p.take(8)) }
    ...
    fun signInWithPin() {
        ...
        if (s.email.isBlank() || s.pin.isBlank() || s.pinTenantId.isBlank()) {
            _uiState.update { it.copy(errorMessage = "Tenant, email, dan PIN wajib diisi") }
            return
        }
        _uiState.update { it.copy(isLoading = true, errorMessage = null) }
        viewModelScope.launch {
            when (val outcome = authRepository.loginWithPin(s.pinTenantId, s.email, s.pin)) {
                is AuthRepository.LoginOutcome.Success ->
                    _uiState.update { it.copy(isLoading = false, signedInUser = outcome.user) }
                is AuthRepository.LoginOutcome.Error ->
                    _uiState.update { it.copy(isLoading = false, errorMessage = outcome.message) }
