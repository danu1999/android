...
const CashFlow = () => {
  ...
  const [formData, setFormData] = useState({ date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0], type: "MASUK", desc: "", amount: 0 });
  ...
  const handleRowDoubleClick = (e, id) => {
    ...
    if (target.closest(".nominal-cell") || target.tagName === "INPUT" || target.tagName === "BUTTON") {
      return;
    }
    handleDelete(id);
  };
    ...
    if (window.confirm("\u26A0\uFE0F Fungsi ini akan menghapus semua data pembayaran faktur yang tidak punya referensi di kas keuangan (data ghost).\n\nData yang terdampak akan dihitung ulang statusnya secara otomatis.\n\nLanjutkan?")) {
      try {
        ...
        if (res.data.cleaned > 0) {
          const detail = (res.data.details || []).map(
            (d) => `  \u2022 Payment ID ${d.payment_id} | Faktur ID ${d.invoice_id} | Rp ${d.payment_amount.toLocaleString("id-ID")}`
          ).join("\n");
...
${detail}`);
  ...
  const handleDownloadCSV = async () => {
    try {
      const response = await api.get("/kas/export/csv", {
        responseType: "blob"
        // 👈 PENTING: Beritahu Axios bahwa ini adalah File, bukan JSON
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `Laporan_Kas_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`);
  ...
  if (loading) return <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "80vh", flexDirection: "column", gap: "15px" }}>
            <div style={{ width: "30px", height: "30px", border: "3px solid #f3f3f3", borderTop: "3px solid #198754", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
            <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
            <div style={{ color: "#6c757d" }}>Memuat data kas...</div>
        </div>;
  return <div style={{ padding: isMobile ? "10px" : "20px" }}>
  ...
  >
    ...
    /* ✅ FIX: Tombol Download CSV diletakkan di sini */
                    ...
                    <button onClick={() => {
    setFormData({ date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0], type: "MASUK", desc: "", amount: 0 });
    setShowModal(true);
  }} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#0d6efd", color: "white", border: "none", padding: "10px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", fontSize: isMobile ? "12px" : "14px", justifyContent: "center" }}>
  ...
  >
                                {editingId === f.ID ? <div style={{ display: "flex", alignItems: "center", gap: "5px", justifyContent: "flex-end" }}>
                                        <input
    type="number"
    value={editAmount}
                    ...
                    <div style={{ background: "white", padding: isMobile ? "20px" : "30px", borderRadius: "24px", width: "100%", maxWidth: "400px", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)" }}>
                        ...
                        <form onSubmit={handleSave}>
                            ...
                            <div style={{ marginBottom: "25px" }}>
                                ...
                                <input type="number" required value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value === "" ? "" : Number(e.target.value) })} style={{ width: "100%", padding: "12px", border: "1px solid #e2e8f0", borderRadius: "12px", outline: "none", fontSize: "16px", fontWeight: "800" }} />
                            </div>
