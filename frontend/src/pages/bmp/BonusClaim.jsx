...
const MACHINES = [
  { name: "Baskom Panda", bonus: 5e3, color: "#3b82f6" },
  { name: "Baskom Mawar", bonus: 5e3, color: "#22c55e" },
  { name: "Baskom Jago", bonus: 7e3, color: "#ef4444" },
  { name: "Baskom Smile 12", bonus: 7e3, color: "#f97316" },
  { name: "Bak Kuping", bonus: 8e3, color: "#a855f7" },
  { name: "Wakul Telor", bonus: 5e3, color: "#eab308" },
  { name: "Baskom Durian", bonus: 5e3, color: "#84cc16" },
  { name: "Wakul Moris", bonus: 5e3, color: "#06b6d4" },
  { name: "Bahtera TM", bonus: 5e3, color: "#ec4899" },
  { name: "BMP", bonus: 5e3, color: "#64748b" }
];
...
const BonusClaim = () => {
  ...
  return <div style={page}>
    ...
    /* Toggle hanya muncul saat step = 'pin' */
                ...
                {step === "pin" && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                        <button
    onClick={() => {
      setView("login");
      setError("");
    ...
    style={{
      ...
      borderRadius: "10px",
      border: "none",
      background: view === "login" ? "linear-gradient(135deg, #3b82f6, #1d4ed8)" : "rgba(255,255,255,0.08)",
      color: "white",
  ...
  >
    ...
    style={{
      ...
      borderRadius: "10px",
      border: "none",
      background: view === "pinlist" ? "linear-gradient(135deg, #7c3aed, #5b21b6)" : "rgba(255,255,255,0.08)",
      color: "white",
  ...
  >
    ...
    /* ── VIEW: Login PIN ── */
            ...
            {step === "pin" && view === "login" && <div style={card}>
  ...
  />
                    <button onClick={handleVerifyPIN} disabled={loading} style={btn("linear-gradient(135deg,#3b82f6,#1d4ed8)")}>
                        {loading ? "Memverifikasi..." : "\u{1F510} Verifikasi PIN"}
                    </button>
                </div>}
    ...
    /* ── VIEW: Daftar PIN ── */
            ...
            {step === "pin" && view === "pinlist" && pinList.length > 0 && <div style={{
    width: "100%",
    maxWidth: "440px",
    background: "rgba(255,255,255,0.05)",
    border: "1px solid rgba(255,255,255,0.1)",
    ...
    /* Kolom kiri */
                        ...
                        <div style={{ borderRight: "1px solid rgba(255,255,255,0.07)" }}>
                            {col1.map((row, i) => <div key={i} style={pinCell(i % 2 === 0)}>
    ...
    /* Kolom kanan */
                        ...
                        <div>
                            {col2.map((row, i) => <div key={i} style={pinCell(i % 2 === 0)}>
                                    <span style={{ color: "rgba(255,255,255,0.8)", fontSize: "11px", fontWeight: 600, textTransform: "capitalize", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginRight: "4px" }}>
                                        {row.name}
                                    </span>
                                    <span style={pinBadge}>{row.pin}</span>
    ...
    /* ── STEP 2: Pilih Mesin & Shift ── */
            ...
            {step === "form" && employee && <div style={{ ...card, overflowY: "auto", flexShrink: 1 }}>
                    {error && <div style={errBox}>⚠️ {error}</div>}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "10px" }}>
                        {MACHINES.map((m) => <button key={m.name} onClick={() => setMachine(m.name)} style={{
    padding: "9px 12px",
    borderRadius: "9px",
    ...
    alignItems: "center",
    fontSize: "13px",
    fontWeight: machine === m.name ? 800 : 500
  }}>
                                <span>{m.name}</span>
    ...
    /* Input Jumlah Perolehan — muncul setelah mesin dipilih */
                    ...
                    {machine && <div style={{ marginBottom: "10px" }}>
  ...
  />
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "6px", marginBottom: "10px" }}>
                        {SHIFTS.map((s) => <button key={s} onClick={() => setShift(s)} style={{
    padding: "9px 4px",
    borderRadius: "9px",
    ...
    cursor: "pointer",
    fontWeight: shift === s ? 800 : 500,
    fontSize: "12px"
  }}>
                                {s === "Pagi" ? "\u2600\uFE0F" : s === "Siang" || s === "Sore" ? "\u{1F324}\uFE0F" : "\u{1F319}"}<br />{s}
                            </button>)}
                    </div>
    ...
    /* ── STEP 3: Sukses ── */
            ...
            {step === "success" && result && <div style={{ ...card, textAlign: "center" }}>
