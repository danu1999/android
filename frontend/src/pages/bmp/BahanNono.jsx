...
const BahanNono = () => {
  ...
  const [tanggal, setTanggal] = useState((/* @__PURE__ */ new Date()).toISOString().split("T")[0]);
  ...
  const openModal = () => {
    ...
    setTanggal((/* @__PURE__ */ new Date()).toISOString().split("T")[0]);
    setNotes("");
    setTagihan("");
  ...
  const handleSave = async (e) => {
    ...
    const payload = tab === "IN" ? { trans_type: "IN", tanggal: tanggal + "T00:00:00Z", notes, items: finalItems, tagihan } : { trans_type: "OUT", tanggal: tanggal + "T00:00:00Z", notes, nominal: Number(nominal), tagihan };
    try {
  ...
  return <div style={{ padding: isMobile ? "12px" : "20px", background: "#f8fafc", minHeight: "100%", maxWidth: "100vw", boxSizing: "border-box", overflowX: "hidden" }}>
            <style>{`
                .btn-primary {
  ...
  >
    ...
    /* Collapsible Calendar Section */
            ...
            {showCalendar && <div style={{ background: "white", borderRadius: "18px", padding: "16px", border: "1px solid #e2e8f0", marginBottom: "16px", boxShadow: "0 4px 10px rgba(0,0,0,0.02)" }}>
                    {
  ...
  >
    ...
    /* Weekdays Header */
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "4px", textAlign: "center", marginBottom: "6px" }}>
                        {["Sen", "Sel", "Rab", "Kam", "Jum", "Sab", "Min"].map((day) => <div key={day} style={{ fontSize: "11px", fontWeight: "700", color: "#94a3b8", padding: "2px 0" }}>{day}</div>)}
                    </div>
    ...
    const isToday = toLocalYYYYMMDD(/* @__PURE__ */ new Date()) === dateStr;
    return <button
      key={dateStr}
      onClick={() => {
        if (isSelected) {
    ...
    /* Active Calendar Filter Alert Banner */
            ...
            {selectedCalendarDay && <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "12px", padding: "10px 14px", marginBottom: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
  ...
  >
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                    {filteredData.map((d) => <div key={d.ID} onClick={() => openDetail(d)} className="tx-card" style={{ background: "white", borderRadius: "16px", padding: "14px", border: "1px solid #e2e8f0", cursor: "pointer" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                    {!d.Tagihan && <div style={{ background: "#fef2f2", color: "#ef4444", padding: "3px", borderRadius: "50%", display: "flex" }} title="Bukti belum diunggah">
                                            <AlertCircle size={12} />
                                        </div>}
    ...
    <div style={{ background: "white", borderRadius: "18px", padding: "8px", boxShadow: "0 1px 3px rgba(0,0,0,0.01)", border: "1px solid #e2e8f0", overflow: "hidden" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "13px" }}>
                        ...
                        <tbody>
                            {filteredData.map((d) => <tr key={d.ID} className="table-row" style={{ borderBottom: "1px solid #f1f5f9" }}>
    ...
    >
            ...
            {filteredData.length === 0 && !loading && <div style={{ textAlign: "center", padding: "50px 20px", color: "#64748b", background: "white", borderRadius: "18px", border: "1px solid #e2e8f0", marginTop: "10px" }}>
    ...
    /* MODAL DETAIL */
            ...
            {showDetailModal && selectedItem && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(15, 23, 42, 0.45)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1200, padding: "15px", backdropFilter: "blur(4px)" }}>
                    <div style={{ background: "white", borderRadius: "20px", width: "100%", maxWidth: "440px", overflow: "hidden", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
                        ...
                        <div style={{ padding: "20px", overflowY: "auto", flex: 1 }}>
                            ...
                            {selectedItem.Items && selectedItem.Items.filter((i) => !i.JenisBahan.startsWith("[JASA]")).length > 0 && <div style={{ marginBottom: "18px" }}>
                                    ...
                                    <div style={{ border: "1px solid #e2e8f0", borderRadius: "12px", overflow: "hidden" }}>
                                        {selectedItem.Items.filter((i) => !i.JenisBahan.startsWith("[JASA]")).map((item, idx, arr) => <div key={item.ID} style={{ display: "flex", justifyContent: "space-between", padding: "10px 12px", borderBottom: idx === arr.length - 1 ? "none" : "1px solid #e2e8f0", background: idx % 2 === 0 ? "white" : "#f8fafc", fontSize: "12px" }}>
                                                <div>
  ...
  >
    ...
    /* MODAL TAMBAH/EDIT */
            ...
            {showModal && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(15, 23, 42, 0.45)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1200, padding: "15px", backdropFilter: "blur(4px)" }}>
                    <div style={{ background: "white", padding: isMobile ? "20px" : "25px", borderRadius: "20px", width: "100%", maxWidth: "480px", maxHeight: "90vh", overflowY: "auto", position: "relative", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" }}>
                        ...
                        <h3 style={{ marginBottom: "20px", fontSize: "18px", fontWeight: "800", color: "#1e293b" }}>{isEditing ? "\u{1F4DD} Edit Transaksi" : "\u{1F4E6} Transaksi Bahan Baku"}</h3>
                        ...
                        <form onSubmit={handleSave}>
                            ...
                            {tab === "IN" && <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: "15px", marginTop: "15px" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                                    ...
                                    {items.map((item, idx) => <div key={idx} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2.2fr 1fr 1.8fr 30px", gap: "6px", marginBottom: "10px", alignItems: "center", borderBottom: isMobile ? "1px solid #f1f5f9" : "none", paddingBottom: isMobile ? "8px" : "0" }}>
                                            <select
    value={item.jenis_bahan}
    onChange={(e) => {
      const n = [...items];
      n[idx].jenis_bahan = e.target.value;
      setItems(n);
    }}
  ...
  >
                                            ...
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 30px", gap: "6px", alignItems: "center" }}>
                                                <input type="number" placeholder="Qty" value={item.kuantitas || ""} onChange={(e) => {
    const n = [...items];
    n[idx].kuantitas = e.target.value === "" ? "" : Number(e.target.value);
    setItems(n);
  }} style={{ width: "100%", padding: "8px", border: "1px solid #cbd5e1", borderRadius: "8px", outline: "none", fontSize: "12px" }} />
                                                <input type="number" placeholder="Harga" value={item.rate || ""} onChange={(e) => {
    const n = [...items];
    n[idx].rate = e.target.value === "" ? "" : Number(e.target.value);
    setItems(n);
  }} style={{ width: "100%", padding: "8px", border: "1px solid #cbd5e1", borderRadius: "8px", outline: "none", fontSize: "12px" }} />
                            ...
                            {tab === "IN" && <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: "15px", marginTop: "15px" }}>
                                    ...
                                    {jasaItems.map((item, idx) => <div key={`jasa-${idx}`} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2.2fr 1fr 1.8fr 30px", gap: "6px", marginBottom: "10px", alignItems: "center", borderBottom: isMobile ? "1px solid #f1f5f9" : "none", paddingBottom: isMobile ? "8px" : "0" }}>
                                            <input type="text" placeholder="Nama Jasa (Titip Giling)" value={item.jenis_bahan} onChange={(e) => {
    const n = [...jasaItems];
    n[idx].jenis_bahan = e.target.value;
    setJasaItems(n);
  }} style={{ width: "100%", padding: "8px", border: "1px solid #cbd5e1", borderRadius: "8px", outline: "none", fontSize: "12px" }} />
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 30px", gap: "6px", alignItems: "center" }}>
                                                <input type="number" placeholder="Berat" value={item.kuantitas || ""} onChange={(e) => {
    const n = [...jasaItems];
    n[idx].kuantitas = e.target.value === "" ? "" : Number(e.target.value);
    setJasaItems(n);
  }} style={{ width: "100%", padding: "8px", border: "1px solid #cbd5e1", borderRadius: "8px", outline: "none", fontSize: "12px" }} />
    ...
    setJasaItems(n);
                            ...
                            {tab === "OUT" && <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: "15px", marginTop: "15px" }}>
                                    <label style={{ display: "block", marginBottom: "6px", fontSize: "12px", fontWeight: "700", color: "#475569" }}>Nominal Pembayaran (Rp)</label>
                                    <input type="number" required value={nominal || ""} onChange={(e) => setNominal(e.target.value === "" ? "" : Number(e.target.value))} style={{ width: "100%", padding: "10px", border: "1px solid #cbd5e1", borderRadius: "10px", outline: "none", fontSize: "15px", fontWeight: "800", color: "#ef4444" }} />
                                </div>}
