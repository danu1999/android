...
const Payroll = () => {
  ...
  const paySalary = async (e) => {
    ...
    try {
      ...
      const empIds = validEntries.map((e2) => e2.employee_id).join(",");
  ...
  const autoFillFromLogs = () => {
    ...
    employees.forEach((emp) => {
      ...
      if (empLogs.length > 0) {
        ...
        while (i < sortedLogs.length) {
          ...
          let late = sortedLogs[i].LateMinutes !== void 0 && sortedLogs[i].LateMinutes !== null ? sortedLogs[i].LateMinutes : getLateMinutes(checkInTime, String(emp.FingerprintPIN));
          totalLateMinutes += late;
          let nextI = i + 1;
          while (nextI < sortedLogs.length) {
            let diffHours = (new Date(sortedLogs[nextI].LogTime).getTime() - checkInTime.getTime()) / (1e3 * 60 * 60);
        ...
        if (daysAttended > 0 && totalLateMinutes > 0) {
          const potongan = totalLateMinutes * 1e3;
    ...
    if (newEntries.length === 0) alert("Tidak ada data absen karyawan minggu ini!");
      ...
      setEntries(newEntries);
      alert(`Berhasil memuat otomatis ${newEntries.length} karyawan!`);
    }
  };
  const getLocalDateString = (timeStr) => {
    const d = new Date(timeStr);
    const wibDateStr = d.toLocaleDateString("sv-SE", { timeZone: "Asia/Jakarta" });
    return wibDateStr;
  };
  ...
  const handleAlasanChange = async (date, pin, value) => {
    try {
      ...
      alert("Gagal menyimpan alasan absensi");
    }
  };
  ...
  const openEditLog = (log, employeePin, date) => {
    if (log && log.ID) {
      ...
      const workDate = log.WorkDate && !log.WorkDate.startsWith("0001") ? log.WorkDate.split("T")[0] : new Date(log.LogTime).toLocaleDateString("sv-SE", { timeZone: "Asia/Jakarta" });
      setEditingLog(log);
  ...
  return <div style={{ padding: "12px", maxWidth: "700px", margin: "0 auto", fontFamily: "system-ui, sans-serif", width: "100%", boxSizing: "border-box", overflowX: "hidden" }}>
  ...
  >
    ...
    >
      ...
      /* Info denda */
                                    ...
                                    {hasLate && emp && <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "6px", background: "#fff3cd", border: "1px solid #fbbf24", borderRadius: "8px", padding: "8px 10px", fontSize: "12px", marginBottom: "10px" }}>
    ...
    /* Summary total semua */
                        ...
                        {entries.length > 1 && <div style={{ background: "linear-gradient(135deg, #1e293b, #334155)", borderRadius: "14px", padding: "14px 16px", marginBottom: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div>
  ...
  >
    ...
    /* Form tambah */
                    ...
                    {showAddEmp && <div style={{ ...cardStyle, border: "2px dashed #93c5fd", background: "#eff6ff" }}>
  ...
  >
    ...
    /* List karyawan sebagai cards */
                    ...
                    {employees.map((emp) => <div key={emp.ID} style={{ ...cardStyle, border: editingId === emp.ID ? "2px solid #0d6efd" : "1px solid #e2e8f0" }}>
                            {editingId === emp.ID ? (
    /* Mode Edit */
    <div>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
    ...
    <div>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                                                {emp.FingerprintPIN && <span style={{ background: "#e0e7ff", color: "#3730a3", borderRadius: "6px", padding: "2px 8px", fontSize: "12px", fontWeight: 700 }}>
                                                        PIN: {emp.FingerprintPIN}
                                                    </span>}
                                                {!emp.IsActive && <span style={{ background: "#fee2e2", color: "#dc2626", borderRadius: "6px", padding: "2px 8px", fontSize: "11px" }}>Nonaktif</span>}
                                            </div>
                                    ...
                                    <div style={{ display: "flex", gap: "6px", marginTop: "12px", flexWrap: "wrap" }}>
                                        <button
      onClick={() => setSelectedEmployeeForLog(emp)}
    ...
    >
                    ...
                    {employees.length === 0 && <div style={{ textAlign: "center", padding: "40px 20px", color: "#94a3b8" }}>
    ...
    /* Filter bar */
                    ...
                    <div style={{ ...cardStyle, display: "flex", gap: "8px", flexWrap: "wrap", padding: "12px" }}>
                        <select style={{ ...inputStyle, flex: 1, minWidth: "140px" }} value={logFilterDate} onChange={(e) => setLogFilterDate(e.target.value)}>
                            ...
                            {getGroupedAttendance().sortedDates.map((d) => <option key={d} value={d}>{(/* @__PURE__ */ new Date(d + "T00:00:00")).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" })}</option>)}
                        </select>
  ...
  >
    ...
    if (filtered.length === 0) {
      return <div style={{ textAlign: "center", padding: "40px 20px", color: "#94a3b8", background: "white", borderRadius: "16px", border: "1px solid #f1f5f9" }}>
    ...
    return Object.entries(byDate).map(([date, rows]) => <div key={date} style={{ marginBottom: "12px" }}>
                                {
      /* Date header */
    }
                                <div style={{ background: "#1e293b", color: "white", borderRadius: "10px 10px 0 0", padding: "8px 14px", fontSize: "13px", fontWeight: 700 }}>
                                    📅 {(/* @__PURE__ */ new Date(date + "T00:00:00")).toLocaleDateString("id-ID", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}
                                </div>
      ...
      return <div key={ri} style={{ padding: "12px 14px", borderBottom: ri < rows.length - 1 ? "1px solid #f1f5f9" : "none" }}>
        ...
        background: hadir ? "#22c55e" : row.alasan ? "#f59e0b" : "#e2e8f0"
                                                    ...
                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                        ...
                                                        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "4px" }}>
                                                            {row.scan1 && <span style={{ background: "#dcfce7", color: "#16a34a", borderRadius: "6px", padding: "2px 8px", fontSize: "12px", fontWeight: 600 }}>
                                                                    Masuk: {row.scan1}
                                                                </span>}
                                                            {row.scan2 && <span style={{ background: "#dbeafe", color: "#1d4ed8", borderRadius: "6px", padding: "2px 8px", fontSize: "12px", fontWeight: 600 }}>
                                                                    Pulang: {row.scan2}
                                                                </span>}
                                                            {!hadir && !row.scan2 && (row.alasan ? <span style={{ background: "#fff3cd", color: "#92400e", borderRadius: "6px", padding: "2px 8px", fontSize: "12px" }}>
                                                                        {row.alasan}
                                                                    </span> : <select
        value={row.alasan}
      ...
      >
        ...
        /* Tombol Edit & Hapus */
                                                    ...
                                                    <div style={{ display: "flex", gap: "4px", flexShrink: 0 }}>
                                                        <button
        onClick={() => {
          const firstLog = row.logIds.length > 0 ? attendanceLogs.find((l) => l.ID === row.logIds[0]) : null;
    ...
    /* Modal Log Absen Pop-up */
            ...
            {selectedEmployeeForLog && <div style={{
    position: "fixed",
    top: 0,
    left: 0,
    right: 0,
  ...
  >
    ...
    if (logsGrouped.length === 0) {
      return <div style={{ textAlign: "center", padding: "30px 10px", color: "#94a3b8" }}>
      ...
      return <div key={idx} style={{
        ...
        boxShadow: "0 1px 3px rgba(0,0,0,0.02)"
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                                <span style={{ fontSize: "13px", fontWeight: 700, color: "#1e293b" }}>
                                                    {(/* @__PURE__ */ new Date(g.date + "T00:00:00")).toLocaleDateString("id-ID", { weekday: "long", day: "2-digit", month: "short", year: "numeric" })}
                                                </span>
                                                <span style={{
        ...
        fontWeight: 700
                                            ...
                                            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center" }}>
                                                {g.scan1 ? <span style={{ background: "#ecfdf5", color: "#065f46", borderRadius: "6px", padding: "4px 8px", fontSize: "11px", fontWeight: 600 }}>
                                                        Masuk: {g.scan1}
                                                    </span> : <span style={{ background: "#f8fafc", color: "#64748b", borderRadius: "6px", padding: "4px 8px", fontSize: "11px", fontStyle: "italic" }}>
                                                        Belum Masuk
                                                    </span>}
                                                
                                                {g.scan2 ? <span style={{ background: "#eff6ff", color: "#1e40af", borderRadius: "6px", padding: "4px 8px", fontSize: "11px", fontWeight: 600 }}>
                                                        Pulang: {g.scan2}
                                                    </span> : <span style={{ background: "#f8fafc", color: "#64748b", borderRadius: "6px", padding: "4px 8px", fontSize: "11px", fontStyle: "italic" }}>
                                                        Belum Pulang
                                                    </span>}
