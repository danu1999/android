...
const CreateInvoice = () => {
  ...
  useEffect(() => {
    if (invoice.client_id !== 0 && clients.length > 0) {
      ...
      if (selectedClient) {
        ...
        const calculatedDate = new Date(Date.now() + daysToAdd * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
  ...
  const handleProductChange = (index, field, value) => {
    ...
    const typedValue = typeof value === "string" && ["master_item_id", "quantity", "jumlah_lusin", "custom_price", "harga_beli"].includes(field) ? value === "" ? "" : Number(value) : value;
  ...
  const handleSave = async (e) => {
    ...
    const validProducts = products.filter((p) => p.master_item_id !== 0 && Number(p.quantity) > 0).map((p) => {
      return {
        ...p,
    ...
    try {
      await api.post("/invoices", {
        ...
        date_created: invoice.date_created ? invoice.date_created + "T00:00:00Z" : (/* @__PURE__ */ new Date()).toISOString(),
  ...
  return <div className="legacy-page" style={{ padding: "20px", maxWidth: "1000px", margin: "0 auto" }}>
    ...
    /* Card 2: Pilih Pelanggan */
                    ...
                    <div style={{ background: "white", padding: "20px", borderRadius: "12px", borderLeft: "5px solid #198754", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
                        ...
                        <div>
                            ...
                            <textarea value={invoice.notes} onChange={(e) => setInvoice({ ...invoice, notes: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #dee2e6", borderRadius: "6px", height: "85px" }} placeholder="Contoh: Barang titipan abah..." />
                        </div>
                    </div>
                </div>
    ...
    /* Card 3: Daftar Barang */
                ...
                <div style={{ background: "white", padding: "25px", borderRadius: "12px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", marginBottom: "30px" }}>
                    ...
                    {products.map((p, i) => <div key={i} style={{ marginBottom: "15px", padding: "15px", border: "1px solid #f1f5f9", borderRadius: "12px" }}>
  ...
  >
    ...
    const masterPrice = master ? master.Price : 0;
    const computedModal = Math.max(0, masterPrice * Number(p.jumlah_lusin) * Number(p.quantity) - Number(p.custom_price) * Number(p.jumlah_lusin) * Number(p.quantity));
    return <div style={{ display: "flex", alignItems: "center", gap: "15px", flexWrap: "wrap" }}>
