...
interface Invoice {
    ...
    Number: string;
    Status: string;
    Client: { ID: number; ClientName: string; PhoneNumber?: string; };
    DateCreated: string;
    DueDate: string;
    Total: number;
    PaidAmount: number;
...
const Invoices: React.FC = () => {
    ...
    const handleMassal = async (e: React.FormEvent) => {
        e.preventDefault();
    ...
    const openEditModal = async (id: number) => {
        try {
            ...
            const invProducts = (res.data?.products || []).map((p: any) => ({
    ...
    const downloadJPG = async (id: number, number: string) => {
        try {
            const res = await api.get(`/invoices/${id}`);
            if (!res.data?.data) {
    ...
    const downloadPDF = (id: number, type: 'pdf' | 'surat-jalan') => {
    ...
    if (loading) return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', flexDirection: 'column', gap: '15px' }}>
            <div style={{ width: '30px', height: '30px', border: '3px solid #f3f3f3', borderTop: '3px solid #dc3545', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
    ...
    return (
        <div className="legacy-page" style={{ padding: isMobile ? '12px' : '30px', background: '#f8f9fa', minHeight: '100%', maxWidth: '100vw', boxSizing: 'border-box', overflowX: 'hidden' }}>
            <InvoiceImageTemplate inv={fullInvoiceData} settings={settings} />

            {previewImage && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '15px', flexDirection: 'column', gap: '15px', backdropFilter: 'blur(6px)' }}>
            ...
            {filterClient !== 'ALL' && clientSummary && (
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '15px', marginBottom: '25px' }}>
                    <div style={{ background: 'white', padding: '20px', borderRadius: '18px', borderLeft: '6px solid #0d6efd', boxShadow: '0 4px 12px rgba(0,0,0,0.02)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button onClick={() => { setMassalData({ ...massalData, client_id: Number(filterClient) }); setShowMassal(true); }} style={{ background: '#0d6efd', color: 'white', border: 'none', padding: '10px 15px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}>
                            Bayar Cepat
                        </button>
                    </div>
            ...
            {isMobile ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {filteredInvoices.map(inv => (
                        <div key={inv.ID} style={{ background: 'white', borderRadius: '18px', padding: '18px', boxShadow: '0 4px 6px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
                ...
                <div style={{ background: 'white', borderRadius: '20px', padding: '10px', boxShadow: '0 4px 12px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9' }}>
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px', textAlign: 'left' }}>
                        ...
                        <tbody>
                        {filteredInvoices.map(inv => (
                            <tr key={inv.ID} style={{ background: '#ffffff' }}>
            ...
            {showEdit && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1200, padding: '10px', backdropFilter: 'blur(4px)' }}>
                    <div style={{ background: 'white', padding: isMobile ? '20px' : '30px', borderRadius: '24px', width: '100%', maxWidth: '650px', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
                        ...
                        <form onSubmit={handleEdit}>
                            {editData.products.map((p, i) => (
                                <div key={i} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1fr 1fr 1.5fr 1.5fr 40px', gap: '10px', marginBottom: '15px', alignItems: 'end', background: '#f8fafc', padding: '15px', borderRadius: '15px' }}>
                                    <div>
