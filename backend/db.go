...
func initSchema() error {
	...
	machineAndMoldMigrations := []string{
		...
		`ALTER TABLE "bmp_production_logs" ADD COLUMN IF NOT EXISTS "machine_id" INT;`,
	...
	bmpManufakturPerfectionMigrations := []string{
		...
		`CREATE TABLE IF NOT EXISTS "bmp_phl_sessions" (
			...
			"shiftName" VARCHAR(100) DEFAULT 'Pagi (07.00 - 15.00)',
			"dailyWage" DOUBLE PRECISION DEFAULT 50000.0,
			"maxQuota" INT NOT NULL DEFAULT 3,
			"registeredCount" INT DEFAULT 0,
			"status" VARCHAR(50) DEFAULT 'OPEN',
			"notes" TEXT DEFAULT '15 menit wajib sudah ada di lokasi, silahkan japri no. wa 082245077959 untuk lokasi',
			"createdAt" BIGINT,
			"updatedAt" BIGINT,
			"isDeleted" BOOLEAN DEFAULT FALSE
		);`,
