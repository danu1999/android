...
# 1. Build React BMP Frontend
Write-Host "Building React BMP Frontend..." -ForegroundColor Cyan
Set-Location bmp-go/golang-frontend
cmd /c "npm run build"
Set-Location ../..

# 2. Sync Frontend build with Go Backend Public directory
Write-Host "Copying frontend build assets to Go backend public..." -ForegroundColor Cyan
...
# Copy build assets
Copy-Item -Path "bmp-go/golang-frontend/dist/*" -Destination $publicDir -Recurse
...
$env:GOOS = "linux"
$env:GOARCH = "amd64"
cmd /c "go build -o invoice-bmp-main main.go"
Set-Location ../..
...
scp -o StrictHostKeyChecking=no -i $pemPath bmp-go/golang-backend/invoice-bmp-main "${user}@${ip}:${vpsPath}/invoice-bmp-main"
...
Write-Host "Extracting assets and restarting PM2 instances on VPS..." -ForegroundColor Cyan
$remoteCommand = "unzip -o ${vpsPath}/public.zip -d ${vpsPath}/public && rm ${vpsPath}/public.zip && pm2 restart bmp-backend && pm2 restart bmp-backend-demo && pm2 status"
ssh -o StrictHostKeyChecking=no -i $pemPath "${user}@${ip}" "$remoteCommand"

# Clean local Linux binary and zip file
Remove-Item bmp-go/golang-backend/invoice-bmp-main
